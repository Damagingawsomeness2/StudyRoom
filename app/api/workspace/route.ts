...
import {lectureOptions} from '@/lib/lecture-options';
...
export async function PUT(request:Request){try{
 ...
 if(hasDraft){const d=x.draft;if(!d||typeof d!=='object'||!Array.isArray(d.files))throw new RequestError('Your draft could not be read.');
  ...
  const draft:BuilderDraft={...(d.editing&&Number.isFinite(d.editing.version)?{editing:{id:validId(d.editing.id),version:d.editing.version}}:{}),subject:str(d.subject,180),focus:str(d.focus,500),depth:d.depth==='quick'?'quick':'deep',model:str(d.model,180),...(Number.isInteger(d.cardTarget)&&d.cardTarget>=5&&d.cardTarget<=200?{cardTarget:d.cardTarget}:{}),notes:str(d.notes,1_000_000),noteTitle:str(d.noteTitle,120),noteOpen:d.noteOpen===true,files:d.files.map((f:Record<string,unknown>)=>{if(!f||typeof f!=='object')throw new RequestError('A draft file could not be read.');const materialId=f.materialId?validId(f.materialId):undefined;return{key:validId(f.key),name:str(f.name,255),screen:f.screen===true,...(f.lectureOptions?{lectureOptions:lectureOptions(f.lectureOptions)}:{}),...(materialId&&ids.has(materialId)?{materialId}:{})};})};
