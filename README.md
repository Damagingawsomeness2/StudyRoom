...
- Subject-only guides use an attributed Wikipedia overview. Add classroom material for teacher-specific content.
- Optional OpenAI generation uses the Responses API with validated structured output. An owner-provided OPENAI_API_KEY must be provisioned server-side through the approved OpenAI Developers setup. Never put API keys in client code or public environment variables. No key is shipped with this project. Custom model IDs must support Responses and structured output. Model API usage is billed to the connected account.

Keep this Site owner-private: the platform access policy protects its files and progress. Supporting multiple independent learners requires per-user records and authorization before expanding access.
