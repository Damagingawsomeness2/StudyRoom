...
export type LectureCheckpoint={version:1;options?:LectureOptions;duration:number;nextSecond:number;segments:SpeechSegment[];unreadable:ReadingIssue[];screen:boolean;screenNext:number;screenUnits:Unit[];screenAttempts:number;speechDone:boolean;screenDone:boolean;parsed?:Parsed;upload?:{id:string;parts:{partNumber:number;etag:string}[]}};
export type LectureJob={data:LectureCheckpoint;save:(patch:Partial<LectureCheckpoint>)=>Promise<void>};
export async function openLectureJob(file:File,screen:boolean,captions:File|undefined,signal:AbortSignal,options?:LectureOptions):Promise<LectureJob>{
 // Hash bounded file samples on the server, including caption content and options.
 const form=new FormData();form.set('name',file.name);form.set('size',String(file.size));form.set('modified',String(file.lastModified));form.set('screen',String(screen));form.set('head',file.slice(0,65536));form.set('tail',file.slice(Math.max(0,file.size-65536)));if(captions)form.set('captions',captions);if(options){form.set('quality',options.quality);form.set('region',screen?options.region:'full');}
 ...
 const job:LectureJob={data:loaded.data||{version:1,options,duration:0,nextSecond:0,segments:[],unreadable:[],screen,screenNext:0,screenUnits:[],screenAttempts:0,speechDone:false,screenDone:!screen},async save(patch){
